...
od_gdf.plot(ax=ax, linewidth=od_gdf['count'] / od_gdf['count'].max() * 50, color='blue', alpha=0.7)

# 设置底图、标题、坐标轴和其他美化
ax.set_title("区与区之间的职住摩擦因子可视化图（线条宽度由'F'决定）", fontsize=15)
ax.set_xlabel("经度")
ax.set_ylabel("纬度")
ax.set_axis_off()

# 保存可视化结果
plt.savefig(os.path.join(output_img_folder, "jobstay_city_F_visualization.png"), dpi=300)
print("地图可视化已保存。")