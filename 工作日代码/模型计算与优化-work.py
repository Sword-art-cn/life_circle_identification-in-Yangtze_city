...
# 导入数据
od_data_path = os.path.join(input_folder, "某工作日跨区出行OD.csv")
area_base_path = os.path.join(output_folder, "workday_area_base.csv")
...
# 保存计算结果
f0_result.to_csv(os.path.join(output_folder, "workday_city_F0.csv"), index=False, encoding="GBK")
od_data[['OAREA', 'DAREA', 'F']].to_csv(os.path.join(output_folder, "workday_city_F.csv"), index=False, encoding="GBK")

#优化结构：将结果重塑为矩阵形式，横轴为 OAREA，纵轴为 DAREA，值为 F
pivot_result = od_data.pivot(index='DAREA', columns='OAREA', values='F')
pivot_result.to_csv(os.path.join(output_folder, "workday_city_F_reshape.csv"), encoding="GBK")
